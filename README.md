...
- `useGeneratedODataFilterQueryString`: interface useful when working with oData service since. Dynamic generator of oData filter query string by using the current container state, and recomputing query string output each time container state change.
